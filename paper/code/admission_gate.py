...
class AdmissionRecord:
    ...
    def to_trace_entry(self) -> MemoryTraceEntry:
        ...
        # Governance fields
        legal_hold = gov.is_legal_hold(m)
        legal_hold_reason = gov.legal_hold_reason(m)
        pinned = gov.is_pinned(m)
        protected = gov.is_protected(m)
        retention_state = gov.retention_state(m)
        retention_policy = retention_state.get("policy")
        retention_expires_at = retention_state.get("expires_at")
